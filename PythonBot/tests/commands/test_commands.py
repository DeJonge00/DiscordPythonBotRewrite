...
class Commands(unittest.TestCase):
    ...
    def test_command_echo(self):
        ...
        self.assertEqual(
            message, bc.command_echo(message.split(), [], author).get(TEXT)
        )
    ...
    def test_command_embed(self):
        ...
        embed = bc.command_embed(
            text.split(), author_name, author_url, [attachment]
        ).get(EMBED)
    ...
    def test_command_emoji(self):
        ...
        for text in [str(emoji_id), emoji_name]:
            ...
            self.assertEqual(
                "https://cdn.discordapp.com/emojis/{}.png".format(emoji_id),
                answer.image.url,
            )
    ...
    def test_command_pat(self):
        ...
        self.assertEqual(
            author.mention + " One does not simply pat ones own head", answer.get(TEXT)
        )
        ...
        for i in range(3):
            answer = get_pat_nr(
                bc.command_pat(time, author, last_pat, target).get(TEXT)
            )
