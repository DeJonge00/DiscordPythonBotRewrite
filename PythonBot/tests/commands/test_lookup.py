...
class Commands(unittest.TestCase):
    ...
    def test_command_osu_empty_user(self):
        with patch('commands.lookup.LookupCommands.command_osu', return_value={'text_message': 'Please provide a username to look for'}):
    ...
    def test_command_osu_random_user(self):
        with patch('commands.lookup.LookupCommands.command_osu', return_value={'text_message': 'Ehhm, there is no user with that name...'}):
            answer = lc.command_osu(['123135456456434234654'], 'author_name', 'url')
            self.assertEqual('Ehhm, there is no user with that name...', answer.get(TEXT))
