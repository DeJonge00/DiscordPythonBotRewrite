...
class Commands(unittest.TestCase):
    def test_welcome_goodbye(self):
        for table in [WELCOME_TABLE, GOODBYE_TABLE]:
            for message in ["Welcome message {} uwu", "owowo"]:
                ...
                self.assertEqual(
                    (CHANNEL_ID, message), welcome.get_message(table, TEST_GUILD_ID)
                )

    def test_do_not_delete(self):
        for i in range(0, 2):
            self.assertEqual(
                dc.toggle_delete_commands("serverid"),
                dc.get_delete_commands("serverid"),
            )
    ...
    def test_banned_command(self):
        for command_name in ["command", "", "1234"]:
            for i in range(2):
                self.assertEqual(
                    bc.toggle_banned_command(SERVER_ID, TEST_GUILD_ID, command_name),
                    bc.get_banned_command(SERVER_ID, TEST_GUILD_ID, command_name),
                )
