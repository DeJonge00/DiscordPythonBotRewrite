...
from database.general.starboard import (
    get_star_channel,
    get_star_message,
    update_star_message,
)
...
async def new_message(bot: PythonBot, message: Message):
    ...
    answer = (
        await react_with_text(bot.pre_command, message, is_private, guild_id, author_id)
        or await react_with_image(bot.pre_command, message, is_private, guild_id)
        or await react_with_action(
            bot.pre_command, message, is_private, guild_id, author_id
        )
        or await talk(
            bot.user if is_private else message.guild.me,
            bot.pre_command,
            message,
            is_private,
            guild_id,
    ...
    if content or embed or image:
        await bot.send_message(
            message.channel, content=content, embed=embed, file=image
        )
        return True
    return False
...
async def react_with_image(
    ...
    if (
        ...
        return {
            EMBED: embedded_pic(
                "Praise the sun!", message.author.avatar_url, praise_the_sun
            ).get(EMBED)
        }
    return {}
...
async def talk(
    me: Member, pre_command, message: Message, is_private: bool, guild_id: int, prefix
):
    """
    ...
    ) or (not is_private and guild_id in constants.bot_talk_blacklist):
        return {}

    if (
        me in message.mentions
        or (
            len(
                set(
                    ...
                    .split(" ")
                ).intersection(set(bot_names))
            )
            > 0
        )
    ) and await pre_command(
        message=message, channel=message.channel, command="talk", delete_message=False
    ):
        if "prefix" in message.content.lower():
            return {TEXT: "My prefix is '{}', darling".format(prefix)}

        if (
            message.author.id in [constants.NYAid, constants.LOLIid, constants.WIZZid]
...
def construct_starboard_embed(message: Message, stars: int):
    ...
    e.set_footer(
        text="Message send at " + message.created_at.strftime("%Y-%m-%d %H:%M:%S")
    )
    return m, e
