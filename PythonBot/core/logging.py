...
async def message(mess: Message, action: str, url=None):
...
async def message_content(content: str, channel: GuildChannel, server: Guild, author: User,
                          timestamp: Message.created_at, raw_mentions: list, action: str, url=None):
    ...
    if action == "pic" or url:
        text = "{} | {} | {} | {} posted a pic : {}".format(timestamp.strftime("%H:%M:%S"), str_cmd(servername),
                                                           str_cmd(channelname), str_cmd(str(author)), str_cmd(url))
    else:
        cont = content
        if public:
