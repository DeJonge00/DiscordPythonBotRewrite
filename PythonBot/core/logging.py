...
def fill_logging_template(guild: str, channel: str, author: str, type: str, content: str):
    ...
    return text.format(guild=str_cmd(guild), channel=str_cmd(channel), author=str_cmd(author), type=str_cmd(type),
...
def fill_announcement_template(guild: str, announcement_text: str):
    ...
    return text.format(guild=str_cmd(guild), announcement_text=str_cmd(announcement_text))
...
def get_channel_and_guild_names(channel: Messageable):
    ...
    """
    return ('DM' if isinstance(channel, DMChannel) else str_cmd(channel.guild.name)), str_cmd(str(channel))
...
def error_on_message(m: Message, error_message: str):
    ...
    log_and_print('errors', fill_logging_template(guild=guild_name, channel=channel_name, author=str(m.author),
                                                  type='Error: ' + error_message, content=m.clean_content))


def error_before_message(destination: TextChannel, author: str, content: str, error_message: str):
    """
    ...
    log_and_print('errors', fill_logging_template(guild=guild_name, channel=channel_name, author=str(author),
                                                  type='Error: ' + error_message, content=content))
