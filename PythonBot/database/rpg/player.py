...
def update_player(player: RPGPlayer):
    get_table(RPG_PLAYER_TABLE).replace_one({USER_ID: str(player.userid)}, player.as_dict(), upsert=True)
...
def add_stats(player_id: int, stat: str, amount: int):
    get_table(RPG_PLAYER_TABLE).update({USER_ID: str(player_id)}, {'$inc': {stat: amount}})


def add_pet_stats(player_id: int, stat: str, amount: int):
    for x in range(len(list(get_table(RPG_PLAYER_TABLE).find({USER_ID: str(player_id)}))[0].get('pets', []))):
        get_table(RPG_PLAYER_TABLE).update({USER_ID: str(player_id)}, {'$inc': {'pets.{}.{}'.format(x, stat): amount}})


def set_stat(player_id: int, stat: str, value):
    get_table(RPG_PLAYER_TABLE).update_one({USER_ID: str(player_id)}, {'$set': {stat: value}})
