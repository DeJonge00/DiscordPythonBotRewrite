...
def get_busy_players():
    return [
        (
            ...
            x.get("stats").get("health"),
        )
        for x in get_table(RPG_PLAYER_TABLE).find(
            {
                "$or": [
                    {"busy.time": {"$lt": 0}},
...
def decrement_busy_counters():
    get_table(RPG_PLAYER_TABLE).update_many(
        {"busy.description": {"$not": {"$eq": BUSY_DESC_NONE}}},
        {"$inc": {"busy.time": -1}},
    )
    ...
    for player in r:
        ...
        if player.get_health() < player.get_max_health():
            if player.busydescription == BUSY_DESC_NONE:
                percentage = (
                    0.025 if player.role == rpgc.names.get("role")[4][0] else 0.01
                )
            else:
                percentage = (
                    0.05 if player.role == rpgc.names.get("role")[4][0] else 0.03
                )
            health = min(
                player.get_max_health(),
                int(player.get_health() + player.get_max_health() * percentage),
            )
...
def get_top_players(group: str, start: int, amount: int):
    ps = (
        get_table(RPG_PLAYER_TABLE)
        .find()
        .sort(group, pymongo.DESCENDING)
        .skip(start)
        .limit(amount)
    )
...
def set_king(user_id: int, server_id: int):
    get_table(RPG_KING_TABLE).update(
        {SERVER_ID: str(server_id)}, {"$set": {USER_ID: user_id}}, upsert=True
    )
