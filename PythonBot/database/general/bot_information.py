...
def server_as_dict(s: Guild):
    ...
    return {
        ...
        CHANNELS: {
            ...
            VOICE_CHANNEL: [
                str(c.id) for c in s.channels if isinstance(c, VoiceChannel)
            ],
        },
    }
...
def channel_as_dict(c: TextChannel):
    ...
    return {NAME: c.name, CHANNEL_ID: str(c.id), CHANNEL_TYPE: channel_type}


def update_channel(channel: TextChannel, channel_table=None):
    if not channel_table:
        channel_table = get_table(CHANNEL_TABLE)
    channel_table.replace_one(
        {CHANNEL_ID: str(channel.id)}, channel_as_dict(channel), upsert=True
    )
...
def update_server(server: Guild, server_table=None, channel_table=None):
    ...
    if not channel_table:
        channel_table = get_table(CHANNEL_TABLE)
    server_table.replace_one(
        {SERVER_ID: str(server.id)}, server_as_dict(server), upsert=True
    )
...
def update_server_member_count(server: Guild, server_table=None):
    if not server_table:
        server_table = get_table(SERVER_TABLE)
    server_table.update_one(
        {SERVER_ID: str(server.id)}, {"$set": {MEMBERS: server.member_count}}
    )
