...
def get_roles(server_id: int):
    ...
    return [int(i) for i in r.keys() if r[i] and re.match('\d\d\d\d\d\d+', i)] if r else []
...
def toggle_role(server_id: int, role_id: int):
    ...
    get_table(SELF_ASSIGNABLE_ROLES_TABLE).update_one({SERVER_ID: str(server_id)}, {'$set': {str(role_id): v}},
                                                      upsert=True)
    return v
