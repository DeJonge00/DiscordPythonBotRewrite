...
def get_star_message(message_id: int):
    r = get_table(STARBARD_MESSAGES_TABLE).find_one({MESSAGE_ID: str(message_id)}, {'embed_id': 1})
    return r.get('embed_id') if r else None


def update_star_message(message_id: int, embed_id: int):
    get_table(STARBARD_MESSAGES_TABLE).update_one({MESSAGE_ID: str(message_id)}, {'$set': {'embed_id': str(embed_id)}},
                                                  upsert=True)
