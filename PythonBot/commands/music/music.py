...
class MusicCommands(Cog):
    ...
    async def music(self, ctx: Context):
        if not ctx.invoked_subcommand:
            ...
            if ctx.message.content in [
                ...
                embed.add_field(
                    name="current",
                    value="Show information about the song currently playing",
                    inline=False,
                )
                embed.add_field(
                    name="join", value="Let me join a voice channel", inline=False
                )
                embed.add_field(
                    name="leave",
                    value="Send me away from the voice channel",
                    inline=False,
                )
                embed.add_field(
                    name="play",
                ...
                embed.add_field(
                    name="repeat", value="Repeat the current song", inline=False
                )
                embed.add_field(
                    name="reset",
                    value="Reset the player for this channel",
                    inline=False,
                )
                embed.add_field(
                    name="skip", value="Vote to skip the current song", inline=False
                )
                embed.add_field(
                    name="stop",
                ...
                embed.add_field(
                    name="volume", value="Change the volume of the songs", inline=False
                )
    ...
    async def join(self, ctx: Context, *args):
        ...
        if ctx.guild.voice_client and ctx.guild.voice_client.is_playing():
            await self.bot.send_message(
                ctx.channel, "Im already singing somewhere else..."
            )
            return

        if ctx.guild.voice_client:
            await self.bot.music_player.join_voice_channel(
                ctx.message, from_state=ctx.guild.voice_client
            )
            return
        await self.bot.music_player.join_voice_channel(ctx.message)

    @music.command(
        name="leave", aliases=["l"], help="Send me away from the voice channel"
    )
    ...
    async def play(self, ctx: Context, *song):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music play"
        ):
            return
        ...
        if state.state.is_playing():
            state.state.pause()
            await self.bot.send_message(
                ctx.channel, ctx.message.author.display_name + " paused my singing..."
            )
            return
        state.state.resume()
    ...
    @music.command(
        name="current",
        aliases=["c"],
        help="Show information about the song currently playing",
    )
    async def current(self, ctx: Context):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music current"
        ):
            return
        state = self.bot.music_player.get_voice_state(ctx.channel)
        if not state or not state.current or not state.is_playing():
            await self.bot.send_message(
                ctx.channel, "I am not performing at the moment"
            )
            return
    ...
    async def queue(self, ctx: Context, *song):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music queue"
        ):
            return
        state = self.bot.music_player.get_voice_state(ctx.channel)
        if not state:
    ...
    async def stop(self, ctx: Context):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music stop"
        ):
            return
        state = self.bot.music_player.get_voice_state(ctx.channel)
        if not state or not state.state.is_playing():
            ...
            return
        await self.bot.music_player.stop_playing(ctx.channel)

    @music.command(name="repeat", aliases=["r"], help="Repeat the current song")
    async def repeat(self, ctx):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music repeat"
        ):
            return
        state = self.bot.music_player.get_voice_state(ctx.channel)
    ...
    @music.command(
        name="skip",
        aliases=["s"],
        help="Vote to skip a song, or just skip it if you are the requester",
    )
    async def skip(self, ctx: Context, *args):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="music skip"
        ):
            return
        state = self.bot.music_player.get_voice_state(ctx.channel)
        if not state or not state.state.is_playing():
            await self.bot.send_message(
                ctx.channel, "I am not playing songs right now..."
            )
            return
        if not ctx.author.voice or ctx.author.voice.channel is not state.state.channel:
            await self.bot.send_message(
                ctx.channel, "You are not in the right voice channel for this command"
            )
            return
        ...
        if len(args) > 0:
            try:
                ...
                await self.bot.send_message(
                    ctx.channel, "I'm not sure that's a number..."
                )
                return
            songs = state.get_queue()
            if n >= len(songs):
                await self.bot.send_message(
                    ctx.channel, "The song queue is not that long..."
                )
            if (ctx.message.author.id == songs[n].requester.id) or (
                ctx.message.author.id == NYAid
            ):
                s = state.remove_song_from_queue(n)
                await self.bot.send_message(
                    ctx.channel, "Removed a song from the queue: {}".format(s)
                )
                return
            else:
                await self.bot.send_message(
                    ctx.channel,
                    "Only the requester, {}, can skip that song".format(
        ...
        votes_needed = ceil(
            len([p for p in ctx.author.voice.channel.members if not p.bot]) / 3
        )
        ...
        if votes >= votes_needed:
            ...
            await self.bot.send_message(
                ctx.channel,
                "{} people voted to skip the song\nSkipping now!".format(votes),
            )
            return
        if state.current.requester.id == ctx.message.author.id:
            state.play_next()
            await self.bot.send_message(
                ctx.channel, "Master has decided to skip this song!"
            )
            return
        await self.bot.send_message(
            ctx.channel, "Votes to skip: {}/{}".format(votes, votes_needed)
        )
