...
logging.basicConfig(
    filename="logs/lookup_commands.log",
    level=LOG_LEVEL,
...
class LookupCommands(Cog):
    ...
    def lookup(
        ...
        n = re.findall(
            search_for + "[0-9]*/", page.text.replace(" ", "").replace("\n", "")
        )
    ...
    @commands.command(
        name="anime", help="Look up an anime!", aliases=["mal", "myanimelist"]
    )
    ...
    def command_osu(args: [str], author_name: str, author_avatar_url: str):
        ...
        params = {"k": osu_api_key, "u": user, "m": 0, "type": "string"}

        r = requests.get(url, params=params)
        if r.status_code != 200:
            return {
                TEXT: "Uh oh, the osu site did not give me any information for my request"
            }
        r = r.json()
        if len(r) <= 0:
        ...
        embed.add_field(
            name="Profile link",
            value="https://osu.ppy.sh/u/{}".format(r.get("user_id")),
        )
        ...
        embed.add_field(
            name="Country rank",
            value="#{} ({})".format(r.get("pp_country_rank"), r.get("country")),
        )
        embed.add_field(
            name="Accuracy", value="{}%".format(round(float(r.get("accuracy")), 3))
        )
        embed.add_field(
            name="Hours played",
            value=str(int(int(r.get("total_seconds_played")) / 3600)),
        )
    ...
    async def osu(self, ctx: Context, *args):
        ...
        answer = LookupCommands.command_osu(
            args, ctx.message.author.display_name, ctx.message.author.avatar_url
        )
    ...
    def command_game(title: str, author: Member):
        ...
        if r.status_code != 200:
            return {
                TEXT: "The description could not be retrieved, something is wrong at the source"
            }
        r = r.json().get("result")
        ...
        if r.status_code != 200:
            return {
                TEXT: "The description for `{}` could not be found, something is wrong at the source".format(
                    title
                )
            }
        r = r.json().get("result")
        if not isinstance(r, dict):
            return {
                TEXT: "The game `{}` exists, but there is no description for it...".format(
                    title
                )
            }
    ...
    def command_urban(args: [str]):
        ...
        try:
            ...
            embed.add_field(
                name="Definition", value="ERROR ERROR ... CANT HANDLE AWESOMENESS LEVEL"
            )
            return {EMBED: embed}

    @commands.command(
        pass_context=1,
        help="Search the totally official wiki!",
    ...
    async def wikipedia(self, ctx, *args):
        ...
        answer = await LookupCommands.command_wikipedia(
            ctx, self.bot.ask_one_from_multiple, args
        )
