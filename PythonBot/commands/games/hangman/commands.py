...
class Hangman(Cog):
    ...
    async def show(
        self, channel: TextChannel, game: HangmanInstance, message="", win=False
    ):
    ...
    async def hangman(self, ctx: Context, *args):
        ...
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="hangman"
        ):
            return
        ...
        if " ".join(args).lower().translate(
            ...
            await self.show(
                ctx.message.channel, game, message=ctx.message.author.name, win=True
            )
            return
        if len(args[0]) > 1:
            game.faults += 1
            ...
            await self.show(
                ctx.message.channel,
                game,
                'Sorry, the word was not "' + " ".join(args) + '"...',
            )
            return
        ...
        if result == WIN:
            self.games.pop(ctx.channel.id)
            await self.show(
                ctx.channel, game, message=ctx.message.author.name, win=True
            )
            return
        if result == RIGHT:
            await self.show(
    ...
    async def new_game(self, ctx: Context, *args):
        if not args:
            await self.bot.send_message(
                ctx.channel, "Please specify 'custom' or 'random'"
            )
            return
    ...
    async def new_random_game(self, ctx: Context, *args):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="hangman new random"
        ):
            return
        ...
        if game:
            await self.bot.send_message(
                ctx.channel, "There is already a game running here..."
            )
            return
    ...
    async def ask_custom_word(self, member: Member):
        ...
        m = await self.bot.wait_for(
            "message",
            check=lambda m: m.author.id == member.id and m.channel.id == c.id,
            timeout=60,
        )
    ...
    async def new_custom_game(self, ctx: Context, *args):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="hangman new custom"
        ):
            return
        game = self.games.get(ctx.channel.id)
        if game:
