...
class TriviaInstance:
    ...
    async def get_params(self, ctx):
        ...
        if self.mode == "turn":
            ...
            await self.bot.send_message(
                ctx,
                "Parameters completed! Each person that now wants to join the game has 30 "
    ...
    async def start_turns_game(self, ctx):
        ...
        for question in questions:
            if not self.keep_playing:
                await self.bot.send_message(
                    ctx, "Game cancelled by {}.".format(self.game_creator.mention)
                )
                return
    ...
    async def stat_time_game(self, ctx):
        ...
        for question in questions:
            ...
            if question["type"] == "multiple":
                answers = "1) {}\n2) {}\n3) {}\n4) {}".format(
                    answers_list[0], answers_list[1], answers_list[2], answers_list[3]
                )
            else:
                answers = "\n".join(answers_list)
    ...
    async def display_question(self, ctx, question, question_nb, answers):
        ...
        embed.add_field(
            name="Question n°", value="{}/{}".format(question_nb, self.questions_nb)
        )
        embed.add_field(name="Category:", value=html.unescape(question["category"]))
        embed.add_field(
            name="Question: ",
            value="**{}**".format(html.unescape(question["question"])),
        )
        embed.add_field(
    ...
    async def ask_target_question(self, ctx, player, question, question_nb):
        ...
        if self.is_answer_correct(
            ...
            await self.bot.send_message(
                ctx,
                player.playerid.mention + " correct! You win 1 point. "
                "Current score: {}".format(player.score),
            )
        else:
            await self.bot.send_message(
                ctx,
                player.playerid.mention + " wrong! The correct answer was: {}. "
                "Current score: {}".format(
                    html.unescape(question["correct_answer"]), player.score
                ),
            )
        ...
        if len(self.players) > 0:
            ...
            while p < 10 and p < len(self.players):
                ...
                long_ass_string += "{}: {} point{}.\n".format(
                    self.players[p].playerid, self.players[p].score, plural
                )
                p += 1
