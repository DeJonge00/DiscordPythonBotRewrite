...
class Minesweeper(Cog):
    ...
    async def minesweeper_new(self, ctx: Context, *args):
        if len(args) < 3:
            await self.bot.send_message(
                ctx.channel, "Please specify height, width and mines"
            )
            return
        game = self.games.get(ctx.channel.id)
        if game:
            await self.bot.send_message(
                ctx.channel, "Another game is busy in this channel..."
            )
            return
    ...
    async def minesweeper_guess(self, ctx: Context, *args):
        ...
        try:
            x, y = (
                min(max(int(args[0]), 1), game.width),
                min(max(int(args[1]), 1), game.height),
            )
        except IndexError:
            await self.bot.send_message(
                ctx.channel, "Please enter the parameters 'x' and 'y'"
            )
            return
        except ValueError:
    ...
    async def minesweeper(self, ctx: Context, *args):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="minesweeper"
        ):
            return
        if not args:
