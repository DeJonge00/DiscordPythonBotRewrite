...
class Trivia(Cog):
    ...
    async def trivia(self, ctx):
        if not await self.bot.pre_command(message=ctx.message, channel=ctx.channel, command="trivia"):
            return
        if not ctx.invoked_subcommand:
            await self.bot.send_message(ctx, "[Usage] To start a new game use: {}trivia new".format(self.bot.get_prefix))
            return
    ...
    async def new(self, ctx):
        if ctx.message.channel in self.game_instances:
            await self.bot.send_message(ctx, "There's already a trivia game on this channel!")
            return
        ...
        if game_mode.content == '1':
            ...
            await self.bot.send_message(ctx, ctx.message.author.mention + " stop wasting my time.")
            return
    ...
    async def join(self, ctx):
        if await self.is_game_running(ctx=ctx, channel=ctx.message.channel, author=ctx.message.author):
            await self.game_instances[ctx.message.channel].player_turn_join(ctx=ctx, author=ctx.message.author)
        return

    @trivia.command(pass_context=1)
    async def quit(self, ctx):
        if await self.is_game_running(ctx=ctx, channel=ctx.message.channel, author=ctx.message.author):
            await self.game_instances[ctx.message.channel].player_quit(ctx=ctx, author=ctx.message.author)
        return
    ...
    async def cancel(self, ctx):
        if await self.is_game_running(ctx=ctx, channel=ctx.message.channel, author=ctx.message.author):
            if self.game_instances[ctx.message.channel].game_creator == ctx.message.author:
                ...
                await self.bot.send_message(ctx, ctx.message.author.mention + " only the game creator can cancel the game")
        return
    ...
    async def is_game_running(self, ctx, channel, author):
        if channel in self.game_instances:
            return True
        await self.bot.send_message(ctx, author.mention + " there's currently no running game on this channel.")
        return False
