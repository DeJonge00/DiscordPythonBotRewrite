...
logging.basicConfig(
    filename="logs/misc_commands.log",
    level=LOG_LEVEL,
...
class MiscCommands(Cog):
    ...
    async def inviteme(self, ctx: Context):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="inviteme"
        ):
            return
    ...
    @commands.command(
        name="helpserver", help="Join my masters discord server for anything"
    )
    async def helpserver(self, ctx: Context):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="helpserver"
        ):
            return
        await self.bot.send_message(
            ctx.channel, "A link to the past:\nhttps://discord.gg/KBxRd7x"
        )

    @commands.command(
        name="vote",
        help="Voting for me helps me help other servers too! Some features are "
        "even multi-server compatible!",
    )
    async def vote(self, ctx: Context):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command="vote"
        ):
            return
        await self.bot.send_message(
            ctx.channel, "Vote for me here:\nhttps://top.gg/bot/244410964693221377"
        )

    async def print_to_admin_channel(self, ctx: Context, type: str, content: str):
        if not await self.bot.pre_command(
            message=ctx.message, channel=ctx.channel, command=type
        ):
            return
    ...
    @commands.command(
        name="suggestion",
        help="Make a suggestion to improve this bot",
        aliases=["suggest"],
    )
    ...
    @commands.command(
        name="complaint",
        help="Make a complaint to the owners of this bot",
        aliases=["complain"],
    )
    ...
    @commands.command(
        name="bugreport",
        help="Notify the owners of this bot of a bug or fault you experienced",
        aliases=["bug", "report"],
    )
