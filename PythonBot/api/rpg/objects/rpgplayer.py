...
class RPGPlayer(rpgcharacter.RPGCharacter):
    def __init__(
        ...
        userid: int,
        username: str,
        picture_url="https://i.pinimg.com/originals/97/7b/b8/977bb8f6f1fc78778f3225ed83b1b4ba.jpg",
        exp=0,
        level=0,
        money=0,
        role=DEFAULT_ROLE,
        ...
        armor=RPGArmor(),
        pets: [RPGPet] = [],
        health=rpgcharacter.DEFAULT_HEALTH,
        maxhealth=rpgcharacter.DEFAULT_HEALTH,
        damage=rpgcharacter.DEFAULT_DAMAGE,
        weaponskill=rpgcharacter.DEFAULT_WEAPONSKILL,
        critical=0,
        bosstier=1,
        kingtimer=0,
        extratime=0,
        ...
        super(RPGPlayer, self).__init__(
            username, picture_url, health, maxhealth, damage, weaponskill, critical
        )
    ...
    def buy_item(self, item: RPGShopItem, amount=1):
        ...
        self.health = min(
            self.get_max_health(),
            rpgcharacter.RPGCharacter.adjust_stats(
                self.get_health(), "health", item, amount=amount
            ),
        )
        self.set_max_health(
            rpgcharacter.RPGCharacter.adjust_stats(
                self.maxhealth, "maxhealth", item, amount=amount
            )
        )
        self.damage = rpgcharacter.RPGCharacter.adjust_stats(
            self.damage, "damage", item, amount=amount
        )
        self.critical = rpgcharacter.RPGCharacter.adjust_stats(
            self.critical, "critical", item, amount=amount
        )
        return True
    ...
    def buy_training(self, item: RPGShopItem, amount=1):
        ...
        self.weaponskill = rpgcharacter.RPGCharacter.adjust_stats(
            self.weaponskill, "weaponskill", item, amount=amount
        )
